...
&nbsp;   \* \*\*Benford’s Law Analysis:\*\* Extracts the `First\_Digit` of transaction amounts to identify potential manual entry manipulation.
...
2\.  \*\*Feature Engineering:\*\* Generates categorical flags like `Is\_Q4` (to track year-end spending) and `Smart Audit Flags`.
...
To run this pipeline locally, you will need a CSV named `pi\_test.csv` (schema: Date, Amount, Currency, Company, Description).
